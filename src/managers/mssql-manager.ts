...
export class MSSQLManager {
  ...
  async validateRuleblock(ruleblock: RuleblockInput): Promise<ValidationResult> {
    ...
    try {
      ...
      // Clean up any existing table (MSSQL uses SROUT_ prefix for output tables)
      const tableName = `SROUT_${ruleblock.name}`;
  ...
  async validateWithDependencies(
    ...
    try {
      ...
      console.log(`📋 Manifest: ${result.manifest.totalRuleblocks} ruleblocks in execution order`);

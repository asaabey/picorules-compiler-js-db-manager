...
DROP TABLE IF EXISTS SROUT_acr_graph;
GO
...
DROP TABLE IF EXISTS #SQ_acr_graph_dv_series;

SELECT
    eadv.eid,
    STRING_AGG(CAST(round(val,0) AS VARCHAR(100)) + '~' + CONVERT(VARCHAR, dt, 23), ',')
    WITHIN GROUP (ORDER BY dt) AS acr_graph_dv_series
INTO
    #SQ_acr_graph_dv_series
FROM
    eadv
WHERE (1=1)
    AND (ATT = 'lab_ua_acr')
...
ALTER TABLE #SQ_acr_graph_dv_series ADD PRIMARY KEY (eid);
...
FROM
    ...
    LEFT OUTER JOIN #SQ_acr_graph_dv_series ON #SQ_acr_graph_dv_series.eid = #UEADV.eid
WHERE (1=1)
;
...
DROP TABLE IF EXISTS #SQ_acr_graph;
...
ALTER TABLE #SQ_acr_graph ADD PRIMARY KEY (eid);
...
SELECT
    ...
    #SQ_acr_graph_dv_series.acr_graph_dv_series,
...
FROM
    ...
    LEFT OUTER JOIN #SQ_line_upper_y ON #SQ_line_upper_y.eid = #UEADV.eid
    LEFT OUTER JOIN #SQ_line_lower_y ON #SQ_line_lower_y.eid = #UEADV.eid
    LEFT OUTER JOIN #SQ_acr_graph ON #SQ_acr_graph.eid = #UEADV.eid
;

ALTER TABLE SROUT_acr_graph ADD PRIMARY KEY (eid);